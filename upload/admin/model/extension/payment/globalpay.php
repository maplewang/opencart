...
class ModelExtensionPaymentGlobalpay extends Model {
	...
	public function addTransaction($globalpay_order_id, $type, $total) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "globalpay_order_transaction` SET `globalpay_order_id` = '" . (int)$globalpay_order_id . "', `date_added` = datetime('now'), `type` = '" . $this->db->escape($type) . "', `amount` = '" . (float)$total . "'");
	}
