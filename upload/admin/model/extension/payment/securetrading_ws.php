...
class ModelExtensionPaymentSecureTradingWs extends Model {
	...
	public function addTransaction($securetrading_ws_order_id, $type, $total) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "securetrading_ws_order_transaction` SET `securetrading_ws_order_id` = '" . (int)$securetrading_ws_order_id . "', `created` = datetime('now'), `type` = '" . $this->db->escape($type) . "', `amount` = '" . (double)$total . "'");
	}
