...
class ModelExtensionPaymentRealex extends Model {
	...
	public function addTransaction($realex_order_id, $type, $total) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "realex_order_transaction` SET `realex_order_id` = '" . (int)$realex_order_id . "', `date_added` = datetime('now'), `type` = '" . $this->db->escape($type) . "', `amount` = '" . (float)$total . "'");
	}
