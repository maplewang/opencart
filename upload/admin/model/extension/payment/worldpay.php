...
class ModelExtensionPaymentWorldpay extends Model {
	...
	public function addTransaction($worldpay_order_id, $type, $total) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "worldpay_order_transaction` SET `worldpay_order_id` = '" . (int)$worldpay_order_id . "', `date_added` = datetime('now'), `type` = '" . $this->db->escape($type) . "', `amount` = '" . (double)$total . "'");
	}
