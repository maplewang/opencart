...
class ModelExtensionPaymentSagepayDirect extends Model {
	...
	public function addTransaction($sagepay_direct_order_id, $type, $total) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "sagepay_direct_order_transaction` SET `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "', `date_added` = datetime('now'), `type` = '" . $this->db->escape($type) . "', `amount` = '" . (float)$total . "'");
	}
