...
class ModelCatalogReview extends Model {
	public function addReview($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "review ( author , product_id , text ,rating ,  status ,date_added ) values('" . $this->db->escape($data['author']) . "', '" . (int)$data['product_id'] . "',  '" . $this->db->escape(strip_tags($data['text'])) . "', '" . (int)$data['rating'] . "', '" . (int)$data['status'] . "',  '" . $this->db->escape($data['date_added']) . "');");
...
//		$this->db->query("UPDATE " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', product_id = '" . (int)$data['product_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "', date_modified = NOW() WHERE review_id = '" . (int)$review_id . "'");
	$this->db->query("UPDATE " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', product_id = '" . (int)$data['product_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "', date_modified = datetime('now') WHERE review_id = '" . (int)$review_id . "'");
		$this->cache->delete('product');
	}
