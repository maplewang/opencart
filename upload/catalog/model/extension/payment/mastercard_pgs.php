...
class ModelExtensionPaymentMastercardPGS extends Model {
	...
	public function saveToken($order_id, $token, $session_id, $sourceOfFunds) {
		...
		if (!empty($response['result']) && $response['result'] == 'SUCCESS' && $response['status'] == 'VALID') {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "mastercard_pgs_token` SET `customer_id`='" . (int)$customer_id . "', `token`='" . $this->db->escape($token) . "', `date_added`=datetime('now')");
		} else {
			...
			if (!empty($response_second['result']) && $response_second['result'] == 'SUCCESS' && $response_second['status'] == 'VALID') {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "mastercard_pgs_token` SET `customer_id`='" . (int)$customer_id . "', `token`='" . $this->db->escape($response_second['token']) . "', `date_added`=datetime('now')");
			}
		}
	}
